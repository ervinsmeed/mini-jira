# Mini Jira

Mini Jira — веб-приложение для управления рабочими пространствами, проектами и задачами на канбан-доске. Интерфейс работает на React, авторизация — через Clerk, данные и серверные проверки доступа — в Convex.

## Реализованные возможности

Перечень подтверждён реализацией frontend и backend; это не отчёт о прохождении всех пользовательских сценариев.

- Регистрация и вход через Clerk, создание/синхронизация пользователя в Convex после входа; редактирование профиля.
- Создание, редактирование и удаление workspace и проектов (в коде проекты называются `boards`), статусы проектов `active`, `completed`, `archived`.
- Канбан-колонки, создание и редактирование задач, перенос задач между колонками и изменение порядка перетаскиванием.
- Описание, приоритет, исполнитель, срок, story points, задачи и эпики со связью задачи с эпиком.
- Поиск по названию, описанию, имени/email автора и исполнителя; фильтры и сортировка по всему списку задач выбранного проекта, показ результатов порциями по 12; массовое изменение и удаление.
- Шаблоны задач, комментарии, журнал активности, накопительный таймер с запуском, паузой и остановкой. Отдельно показаны текущий/последний сеанс и общий итог; архива сеансов нет.
- Избранные проекты и задачи, недавно просмотренные задачи, аналитика проекта и диаграмма задач по доступным проектам рабочего пространства.
- Добавление зарегистрированных пользователей в workspace и проекты по email; пользовательские роли и серверные проверки разрешений.
- Светлая/тёмная тема, русские и английские переводы, обновление данных через подписки Convex.
- Маршрутизация React Router: у проекта, аналитики и профиля собственные адреса (`/projects/:id`, `/projects/:id/analytics`, `/profile`), работают кнопки «назад/вперёд» браузера и прямые ссылки; неавторизованный пользователь перенаправляется на `/login` и после входа возвращается на запрошенную страницу.
- Формы задач на React Hook Form: общий набор полей для создания и редактирования, валидация названия, подзадачи через `useFieldArray` с перетаскиванием.
- Собственный UI-кит на SCSS-модулях (`Button`, `Input`, `InputNumber`, `Textarea`, `Select`, `Field`, `Modal`, `Skeleton`) поверх токенов темы.
- Skeleton-загрузка доски, ленивые страницы аналитики и профиля (code splitting), `ErrorBoundary` вокруг запросов.

## Стек

Версии ниже — диапазоны из `package.json`; точные версии установки фиксирует `package-lock.json`.

| Назначение | Зависимости |
| --- | --- |
| Интерфейс | React / React DOM `^19.2.7`, TypeScript `~6.0.2` |
| Сборка | Vite `^8.1.1`, `@vitejs/plugin-react` `^6.0.3`, `vite-tsconfig-paths` `^6.1.1` |
| Backend и база данных | Convex `^1.42.1` |
| Авторизация | `@clerk/clerk-react` `^5.61.8`, `@clerk/localizations` `^3.37.8` |
| Стили и UI | Tailwind CSS `^4.3.2`, Radix UI, shadcn, Lucide React, Sonner, Geist; clsx, class-variance-authority, tailwind-merge |
| Перетаскивание | `@dnd-kit/core`, `@dnd-kit/sortable`, `@dnd-kit/utilities` |
| Графики | Recharts `^3.10.1` |
| Глобальное состояние UI | Zustand `^5` с middleware `persist` |
| Формы | React Hook Form `^7.88` |
| Маршрутизация | React Router DOM `^7` |
| Препроцессор стилей | Sass `^1` (SCSS-модули для UI-кита) |
| Локализация | i18next `^26.3.6`, react-i18next `^17.0.11`, browser language detector |
| Проверка кода | ESLint `^10.6.0`, typescript-eslint, плагины React Hooks и React Refresh |

## Структура проекта

```text
src/
  main.tsx, LocalizedApp.tsx  # BrowserRouter → ClerkProvider → ConvexProviderWithClerk → App
  App.tsx                    # Маршруты и защита страниц
  store/uiStore.ts           # Zustand: тема, сайдбар, выбранный workspace
  hooks/                     # useAppRoute, useBoardFilters, useBoardDnd, useTaskPages, useAction, useNow
  components/
    auth/                    # Страница входа и охранники маршрутов
    layout/                  # Каркас приложения, загрузка списков
    board/                   # Доска, шапка, колонки, карточки, фильтры, skeleton
    modals/                  # Формы задач и окна
      task-modal/            # Части окна задачи: таймер, подзадачи, комментарии, действия
    workspace/               # Сайдбар, элемент проекта, подвал
    analytics/               # Дашборд и графики
    ui/kit/                  # Собственный UI-кит на SCSS-модулях
    ui/                      # Обёртки над Radix (Dialog, Select, Checkbox)
  lib/                       # Утилиты frontend
  i18n.ts, locales/          # Переводы и настройка языка
  index.css                  # Токены тем и базовые стили
convex/                      # Backend: запросы и мутации Convex
  schema.ts                  # Таблицы и индексы
  auth.config.ts             # Проверка JWT Clerk
  users.ts, workspaces.js, boards.js, columns.js, tasks.ts
  workspaceMembers.js, boardMembers.js, roles.js
  taskTemplates.js, favorites.js, recentTasks.js, analytics.ts
  lib/                       # Общие проверки доступа, пагинация, каскадное удаление
  _generated/                # Генерируемый Convex API и типы
tests/                       # Юнит-тесты серверных обработчиков и клиентской пагинации
.github/workflows/ci.yml     # CI: lint, тесты, сборка
package.json                 # Зависимости и npm-команды
vite.config.ts               # Конфигурация сборки frontend
```

## Архитектурные решения

**Состояние разделено по источнику.**

| Что | Где живёт | Почему |
| --- | --- | --- |
| Серверные данные: задачи, проекты, права | Convex | реактивные подписки, кеш и обновление в реальном времени на стороне клиента Convex |
| Что открыто на экране: проект, аналитика, профиль | React Router (адрес страницы) | адресом можно поделиться, работают «назад/вперёд» и перезагрузка |
| Оформление: тема, свёрнутый сайдбар, выбранный workspace | Zustand + `persist` | нужно многим компонентам и должно переживать перезагрузку |
| Состояние форм и открытых окон | React Hook Form и локальный `useState` | живёт только пока открыто окно |

**Почему не TanStack Query и Axios.** Convex — не REST API: клиент держит подписки на запросы, сам кеширует результаты и пересчитывает их при изменении данных на сервере, а мутации типизированы сгенерированным API. Слой React Query поверх этого дублировал бы кеш и инвалидацию, а Axios — HTTP-транспорт, которого у Convex нет. Если бы backend был REST, обе библиотеки были бы уместны: React Query для серверного состояния, Axios для запросов и перехватчиков.

**Темы через токены.** Цвета описаны переменными CSS в `src/index.css` (`--surface-1…3`, `--text-primary`, `--brand`, `--danger`), светлая тема переопределяет их в `[data-theme="light"]`. Компоненты не знают о теме: в коде нет проверок вида `theme === "dark"`, а UI-кит на SCSS-модулях использует те же переменные.

**Декомпозиция.** Крупные экраны разобраны на компоненты и хуки: доска — на `BoardHeader`, `BoardSkeleton`, `useBoardFilters` и `useBoardDnd`; окно задачи — на таймер, подзадачи, комментарии, назначение и меню действий; сайдбар — на элемент проекта и подвал. Логика перетаскивания вынесена в хук, а расчёт позиции (`calculateNewOrder`) — в чистую функцию: при переносе меняется `order` только у перемещённой задачи.

## Непрерывная интеграция

`.github/workflows/ci.yml` запускает на каждый push и pull request: `npm ci`, `npm run lint`, `npm test`, `npm run build`. Сборке передаются фиктивные `VITE_*` переменные: сборка frontend не обращается к Convex и Clerk, поэтому реальные ключи в CI не нужны.

Frontend вызывает Convex через сгенерированный API. Backend выполняется в отдельном deployment Convex; собственного Express-сервера в репозитории нет. Clerk подтверждает личность, а прикладные роли хранятся в Convex, не в Clerk Organizations.

## Запуск после клонирования

### 1. Подготовить окружение

Нужны Git, Node.js с npm, доступ к интернету и собственные аккаунты Clerk и Convex. Используйте Node.js 24.x. По `engines` установленных зависимостей Vite требует `^20.19.0 || >=22.12.0`, ESLint — `^20.19.0 || ^22.13.0 || >=24`, Convex — Node.js `>=18` и npm `>=7`. Поэтому для ветки 22 нужен минимум **22.13**, а не просто Node.js 18+.

В каталоге клонированного репозитория:

```sh
node --version
npm --version
npm ci
```

### 2. Создать собственное приложение Clerk

В [Clerk Dashboard](https://dashboard.clerk.com/) создайте приложение для разработки, включите регистрацию по email и подтверждение адреса. В разделе API keys возьмите Publishable Key и Frontend API URL своего приложения. Secret Key для текущего frontend не требуется.

### 3. Настроить токен для Convex

В Clerk откройте **JWT templates → New template**, выберите Convex (или пустой шаблон), задайте имя **`convex`** и сохраните claims:

```json
{
  "aud": "convex",
  "email": "{{user.primary_email_address}}",
  "email_verified": "{{user.email_verified}}",
  "name": "{{user.full_name}}"
}
```

`aud` должен совпадать с `applicationID: "convex"`. Clerk сам добавляет `iss` и `sub`. Значение shortcode сохраняет тип исходного поля: после подтверждения email claim `email_verified` должен быть логическим **`true`**, а не строкой `"true"`. Не заменяйте проверку адреса константой `true`. Настройка шаблонов описана в [официальной документации Clerk](https://clerk.com/docs/guides/sessions/jwt-templates).

В `convex/users.ts` мутация `users.create` требует непустой `email` и `identity.emailVerified === true` (это JWT claim `email_verified`). Адрес нормализуется; привязка одного email к разным Clerk ID запрещена. Одной успешной авторизации в Clerk недостаточно, если этих claims нет: синхронизация пользователя завершится ошибкой.

Актуальные руководства предлагают новый путь: **Activate Convex integration** в Clerk и дополнительные claims в **Sessions → Claims**. Установленный Convex поддерживает оба варианта: при `sessionClaims.aud === "convex"` использует session token, иначе запрашивает JWT template `convex`. Если активируете интеграцию, добавьте `email` и `email_verified` также в session claims: изменение только шаблона тогда не поможет. См. [интеграцию Clerk](https://clerk.com/docs/guides/development/integrations/databases/convex).

### 4. Указать свой issuer в backend

В `convex/auth.config.ts` уже записан фиксированный домен чужого Clerk-приложения. В своей копии замените значение `domain` на **Frontend API URL собственного Clerk**, включая `https://`; `applicationID` оставьте равным `convex`:

```ts
export default {
  providers: [
    {
      domain: "https://<your-clerk-instance>.clerk.accounts.dev",
      applicationID: "convex",
    },
  ],
};
```

Это issuer токена, а не адрес frontend или deployment Convex. Последовательность «настройка Clerk → auth.config → синхронизация Convex» соответствует [руководству Convex](https://docs.convex.dev/auth/clerk).

### 5. Создать и связать Convex deployment

Из корня репозитория выполните:

```sh
npx convex dev --once
```

Авторизуйтесь в своём аккаунте Convex, создайте новый проект и выберите собственный development deployment. CLI свяжет каталог с проектом, загрузит схему, функции и auth config, сгенерирует API и запишет параметры подключения. Используйте собственный облачный deployment для этого сценария. Первый запуск с `--once` позволяет закончить настройку до запуска двух постоянных процессов; такой порядок инициализации приведён в [инструкции Clerk](https://clerk.com/docs/guides/development/integrations/databases/convex).

### 6. Настроить `.env.local`

В корне проекта добавьте или обновите переменные, сохранив параметры, созданные CLI Convex:

```dotenv
VITE_CONVEX_URL=https://<your-dev-deployment>.convex.cloud
VITE_CLERK_PUBLISHABLE_KEY=<your-clerk-publishable-key>
```

Замените placeholders собственными значениями: URL deployment берётся из Convex, Publishable Key — из того же Clerk-приложения, чей domain указан выше. Не используйте URL `convex.site` вместо URL API deployment.

Обе переменные читает `src/main.tsx`. Значения `VITE_*` попадают в браузерную сборку: помещать туда пароли, Secret Key и deploy-токены нельзя. `.env.local` не нужно коммитить. После изменения переменных перезапустите Vite.

### 7. Запустить два терминала

Терминал 1, из корня проекта:

```sh
npx convex dev
```

Оставьте процесс работающим: он синхронизирует backend с development deployment.

Терминал 2, из того же каталога:

```sh
npm run dev
```

Откройте адрес из вывода Vite (обычно `http://localhost:5173`), зарегистрируйтесь и подтвердите email. После входа приложение вызовет `users.create`. Если рабочие пространства не загружаются, проверьте совпадение Clerk domain, audience и наличие обязательных claims, не публикуя JWT и ключи.

## Команды

| Команда | Назначение |
| --- | --- |
| `npm run dev` | Сервер разработки Vite |
| `npx convex dev` | Разработка и синхронизация backend Convex |
| `npm run build` | `tsc -b && vite build`, сборка frontend в `dist/` |
| `npm run lint` | `eslint .` |
| `npm test` | Юнит-тесты серверных обработчиков и клиентской пагинации |
| `npm run typecheck` | Проверка типов frontend и проекта Convex |
| `npm run preview` | Локальный просмотр предварительно собранного `dist/` |

Для preview сначала выполните `npm run build`. **Preview не запускает и не развёртывает backend**: браузер обращается к Convex URL, включённому в сборку. Это не production-сервер.

## RBAC по коду

- **Workspace owner** — `workspaces.ownerId`. Может изменять/удалять workspace, управлять его участниками и ролями; получает доступ владельца к проектам внутри workspace.
- **Project owner** — `boards.userId`. Получает доступ владельца к своему проекту, но для проекта с workspace сначала требуется действующий доступ к родительскому workspace. Это не делает его владельцем workspace или управляющим его ролями.
- Роли создаются вручную и принадлежат конкретному workspace. Предустановленных Admin/Editor/Viewer и фиксированных уровней нет. `roleId` назначается отдельно в `workspaceMembers` и `boardMembers`; права двух назначений автоматически не объединяются. Участник без роли не получает разрешений.
- Уровень — конечное число: большее число означает более высокий уровень, но само по себе не добавляет разрешений. Не-владелец может делегировать только роль строго ниже своей и только с подмножеством собственных разрешений. Нельзя управлять участниками/ролями равного или более высокого уровня, назначать роль себе или повышать назначенную себе роль. Назначенную участникам роль удалить нельзя.

| Разрешение | Где применяется |
| --- | --- |
| `project.create` | Роль workspace: создание проекта |
| `project.view` | Роль проекта: видимость проекта в workspace |
| `project.update`, `project.delete` | Роль проекта: изменение/порядок и удаление проекта |
| `task.view` | Чтение задач, колонок, комментариев, активности и шаблонов |
| `task.create`, `task.update`, `task.delete` | Операции над задачами и шаблонами; `task.update` также требуется для переноса, таймера и добавления комментария |
| `members.manage` | Управление участниками соответствующего workspace или проекта |
| `roles.manage` | Управление определениями ролей через роль workspace |
| `analytics.view` | Просмотр аналитики проекта |

Колонки проверяются отдельно: создавать, изменять и удалять их может владелец workspace или владелец проекта при действующем доступе к workspace. Одного `project.update` недостаточно. Шесть колонок по умолчанию создаются на сервере вместе с проектом, одной транзакцией.

Удаление участника из workspace удаляет и его `boardMembers` во всех проектах этого workspace. Дополнительная проверка родительского доступа блокирует доступ даже при оставшейся старой записи проектного членства. Владельца workspace удалить нельзя; удаление участника, владеющего проектом в нём, блокируется. Владельцев workspace/проекта нельзя удалить или переназначить им роль через управление участниками проекта.

## Ручная проверка двумя аккаунтами

Используйте отдельные профили браузера или обычное и приватное окно. Ниже — сценарий для выполнения вручную, а не уже пройденный автоматизированный тест.

1. **Аккаунт A:** зарегистрируйтесь, подтвердите email и войдите. Создайте workspace, проект в нём, колонки и несколько задач. A должен оставаться владельцем проекта.
2. **Аккаунт B:** зарегистрируйте другого пользователя, подтвердите его email и хотя бы один раз войдите в Mini Jira, чтобы появилась запись в `users`. Добавление по email ищет уже существующего пользователя; отправка приглашения незарегистрированному пользователю не реализована.
3. **Аккаунт A:** в управлении ролями workspace создайте роль `Viewer`, например уровня `10`, с разрешениями только `project.view` и `task.view`. При необходимости проверки аналитики отдельно добавьте `analytics.view`.
4. Добавьте B по его подтверждённому email в участники workspace, затем назначьте `Viewer`. Отдельно добавьте B в участники проекта и назначьте ту же роль там. Членство в workspace не добавляет пользователя во все проекты автоматически.
5. **Аккаунт B:** обновите страницу; убедитесь, что проект и задачи видны. Проверьте отсутствие возможности создать, изменить, удалить или перетащить задачу, добавить комментарий, запустить таймер, менять проект, колонки, участников и роли. Элементы управления должны быть скрыты/недоступны; попытка запрещённой мутации от сессии B должна отклоняться сервером. Проверять только скрытие кнопок недостаточно. Личное избранное и история просмотров не считаются изменением общих задач.
6. Оставьте у B открытый проект. **Аккаунт A:** удалите B **только из workspace**, не удаляя его отдельно из проекта.
7. **Аккаунт B:** проверьте отзыв доступа в открытом окне и после обновления страницы: workspace и проект больше не доступны, чтение/изменение прежних задач не должно возвращать защищённые данные или проходить успешно. Старый экран может сохранять уже загруженные данные до обновления; критерий — отсутствие нового серверного доступа.
8. **Аккаунт A:** убедитесь, что B исчез и из участников проекта, а сам проект и задачи сохранились.

## Известные ограничения

- Текущий результат ESLint: **0 ошибок и 0 предупреждений**. Это не заменяет ручную проверку сценариев в браузере.
- Известны предупреждения сборки о крупных чанках и `vite-tsconfig-paths`; оптимизация сборки остаётся отдельной задачей.
- Воспроизводимые изолированные тесты запускаются через `npm test`. Они подменяют базу данных и регистрацию Convex, поэтому не заменяют проверку deployment и браузера.
- Доска использует ограниченный серверный запрос `tasks.page`, индексную сортировку и курсор. Поиск подстроки проверяется сервером для очередного диапазона; все диапазоны доступны через продолжение. Страницы поиска могут быть неполными или пустыми до нахождения совпадений. Это последовательный ограниченный поиск, не индексный полнотекстовый поиск и не гарантия заполненной страницы за один запрос.
- Фильтры задач: колонка, исполнитель (включая отсутствие исполнителя), приоритет, SP и срок. Сортировки: ручной порядок, название (порядок строк индекса Convex), срок по возрастанию, дата создания и SP по убыванию, приоритет. Фильтры сроков используют границы локального календарного дня; аналитика считает просрочку по текущему моменту и исключает завершённые задачи.
- У проектов реализованы ручной порядок, избранное и изменение статуса. Отдельного поиска, фильтра статусов и выбора сортировки проектов в интерфейсе нет. Оригинал ТЗ в доступных файлах отсутствует; соответствие неизвестным требованиям не подтверждено.
- Clerk получает официальный перевод через `ClerkProvider.localization`. Новые события истории и ошибки используют коды; даты следуют языку приложения. Старые текстовые подробности истории сохранены, в том числе английские: они не переводятся с помощью разбора произвольного текста.
- Браузерная проверка текущих исправлений, вход с чистого клона и проверка двух реальных аккаунтов остаются ручными этапами приёмки.
- Аналитика считает завершёнными задачи в колонках с именем `done` или `готово`; если такой колонки нет, завершённой считается последняя колонка доски. Текущий момент времени передаётся с клиента, чтобы счётчик просроченных обновлялся без изменения данных.
- Удаление владельца проекта из workspace блокируется сообщением о необходимости передачи владения, но отдельный сценарий передачи владения в интерфейсе/API не реализован.
- В `auth.config.ts` домен Clerk зафиксирован в коде; автоматического переключения issuer между dev и production сейчас нет.

## Что настроить отдельно для production

Создайте production instance Clerk с собственным доменом, DNS и настройками способов входа/подтверждения email. Перенесите настройку токенов с audience `convex` и обязательными claims, используйте production Publishable Key. Настройте production deployment Convex и его issuer для production Clerk; development domain из репозитория для этого не подходит.

Разделение issuer по окружениям потребует отдельной настройки: например, чтения переменной в `auth.config.ts` и задания её в соответствующих deployments Convex. Одно добавление переменной в `.env.local` не изменит текущий фиксированный `domain`. Развёртывание backend выполняется через `npx convex deploy`; подробнее — [dev/prod-настройка Convex и Clerk](https://docs.convex.dev/auth/clerk#configuring-dev-and-prod-instances).

Для frontend настройте хостинг с HTTPS, Node.js совместимой версии, `npm ci`, `npm run build` и публикацию `dist/`. До сборки задайте production `VITE_CONVEX_URL` и `VITE_CLERK_PUBLISHABLE_KEY` в окружении хостинга. Backend развёртывается отдельно; `npm run build` его не публикует. См. [развёртывание Convex](https://docs.convex.dev/production/overview).

Отдельно настройте CI/CD, хранение deploy-ключей в секретах платформы, мониторинг ошибок, резервное копирование/восстановление данных и повторите ручную проверку двумя аккаунтами на production-конфигурации. Готовый production-процесс этим репозиторием не предоставляется.

### Развёртывание на Vercel

1. В Convex Dashboard откройте **Settings → Deploy keys** и создайте ключ production.
2. В проекте Vercel добавьте переменные окружения: `CONVEX_DEPLOY_KEY` (ключ из шага 1), `VITE_CLERK_PUBLISHABLE_KEY` (production-ключ Clerk). `VITE_CONVEX_URL` задаёт сам `convex deploy` во время сборки.
3. Build command: `npx convex deploy --cmd 'npm run build'` — сначала публикуются функции и схема Convex, затем собирается frontend с URL production-deployment.
4. Output directory: `dist`.
5. `vercel.json` в репозитории перенаправляет все маршруты на `index.html`, иначе прямые ссылки вида `/projects/:id` возвращали бы 404.
6. В production-приложении Clerk укажите домен сайта в разрешённых, а в `convex/auth.config.ts` — Frontend API URL production-инстанса Clerk.

## Дальнейшие шаги

Осознанно отложено, чтобы не переписывать рабочий код перед сдачей:

- **Feature-Sliced Design.** Сейчас файлы сгруппированы по типу (`components/board`, `components/modals`, `hooks`). Следующий шаг — слои `app / pages / widgets / features / entities / shared`: доска и окно задачи становятся widgets, роли и права — entities, UI-кит переезжает в `shared/ui`.
- **Перевод оставшихся модулей Convex с JavaScript на TypeScript.** Задачи, аналитика и общие функции доступа уже на TS; девять файлов остаются на JS, из-за чего типы прав и документов на границе с frontend выводятся слабее.
- **Полный перевод интерфейса на собственный UI-кит.** Сейчас на нём формы задач, шапка доски, фильтры и окна; остальные экраны используют Tailwind.
- **Оптимизация бандла:** разделение вендорных чанков, виртуализация длинных списков задач.
- **E2E-тесты** сценариев двух аккаунтов вместо ручной проверки.

## Проверки доработок и совместимость

Для применения новых функций и индексов запустите `npx convex dev` в собственном development deployment. Сборка frontend сама backend не развёртывает. Во время доработки deployment и реальные данные не изменялись.

- `npm test` — постоянные тесты фактических серверных обработчиков с изолированной базой в памяти. Подменяются регистрация Convex, валидаторы и операции базы. Проверяются каскады, связи эпиков, дедлайн, права, закрытая аналитика, таймер, история, поиск за первой страницей и профиль. Отдельные клиентские тесты запускают установленный `usePaginatedQuery` и `useTaskPages` с подменёнными React-планировщиком, таймерами и транспортом подписок: пустые диапазоны, продолжение, отсутствие дублей, смена запроса и запоздалый ответ. Это не тесты реального движка Convex или браузера; поведение нативных индексов на изолированном deployment не проверялось.
- `npm run typecheck` — frontend со `strict: true` и отдельная проверка проекта Convex. Задачи, аналитика, профиль и новые общие функции написаны на TypeScript. Остальные JS-модули пока не проверяются с `checkJs`; типизация их API остаётся слабее.
- `npm run lint`, `npm run build`, `git diff --check` — итоговые проверки. Успешная сборка не доказывает правильность настроек Clerk/Convex на конкретном deployment.

### Таймер и история

`timerElapsedMs` — зафиксированный общий итог; во время работы интерфейс добавляет текущий отрезок. Необязательное `timerSessionElapsedMs` хранит время текущего/последнего сеанса. Пауза сохраняет сеанс, стоп завершает его, новый запуск начинает сеанс с нуля, сохраняя общий итог. У старых записей длительность сеанса неизвестна и так и подписана до начала нового сеанса; общий итог не сбрасывается.

Показаны даты создания/обновления задачи. Фактические изменения полей записываются как `task.updated` со структурированными `changes`; отсутствие изменения не меняет дату и не добавляет событие. Комментарий обновляет дату задачи. Пользовательские значения и старые `details` сохраняются.

### Ограничения выдачи списков

Доска запрашивает по 12 кандидатов, сервер ограничивает запрашиваемый размер 50. Реактивная страница Convex может расти: размер запроса не является жёстким пределом размера страницы. Основное чтение задач ограничено `maximumRowsRead: 500` и `maximumBytesRead: 1 MiB`; метаданные разбиения страниц передаются клиенту. Связанные пользователи, права и избранное требуют дополнительных чтений: до двух пользователей и одной записи избранного на кандидата, плюс проверки доступа. Пользователи читаются по ID с кешем внутри запроса, а не всей таблицей. Сроки без значения находятся в конце; приоритеты идут high → medium (включая отсутствие значения) → low, неизвестные значения старых записей — в конце. Перенос задачи вычисляет соседнюю позицию на сервере и учитывает непоказанные задачи.

Сохранён регистронезависимый поиск произвольной подстроки названия, описания, имени/email автора и исполнителя. Фильтры применяются перед `paginate`; подстрока проверяется на сервере в очередном диапазоне. Полнотекстовый индекс не подменяет эту семантику. Преобразование страницы и продолжение курсором поддерживаются [пагинацией Convex](https://docs.convex.dev/database/pagination).

Пока совпадений нет, клиент автоматически проходит следующие диапазоны с паузой 150 мс между запросами. После появления результатов продолжение запускается кнопкой «Продолжить поиск». Пока остались диапазоны, интерфейс явно сообщает о незавершённом поиске; «ничего не найдено» показывается только после завершения. Изменение запроса, фильтров или сортировки сбрасывает подписки и отменяет запланированное продолжение; результаты старого запроса не добавляются в новый. Повторяющиеся ID не отображаются дважды.

Стоимость поиска линейно растёт с числом просматриваемых задач, а отсутствие совпадений требует пройти проект до конца. Редкие совпадения означают много запросов, страницы не обязаны быть заполнены. Проверки прав выполняются для каждого запроса. Нативные курсоры сохраняют порядок и продолжение, а `usePaginatedQuery` сохраняет подписки на загруженные диапазоны: объём клиентской памяти и стоимость реактивных пересчётов также растут. Смена запроса или уход с доски прекращают продолжение. Для очень больших проектов это остаётся ограничением масштабирования, а не индексным поиском за постоянное число чтений.

Комментарии и история выдаются по 20; рабочие пространства, проекты, участники, роли, шаблоны и эпики — по 30 с продолжением. В первую страницу участников дополнительно включается владелец. Недавние задачи намеренно показывают последние 8 доступных из проверяемых последних 50 записей. Признак избранного загружается вместе с очередной страницей задач/проектов. Перенос проекта учитывает фактического соседа на сервере, включая незагруженные проекты.

Колонки проекта загружаются целиком для доски и выбора статуса; список ролей текущего пользователя в профиле агрегирует его доступные членства целиком. Старые API полной выдачи сохранены для совместимости, основной интерфейс использует новые страницы. Аналитика агрегирует задачи доступных проектов чтением; постоянных агрегатных счётчиков нет. Большие каскадные удаления ограничены лимитами одной транзакции Convex. Эти операции не предназначены для неограниченных объёмов данных.

### Изменения схемы

Добавлены необязательные `tasks.timerSessionElapsedMs` и `activityLogs.changes`, индексы задач по порядку, колонке/порядку, названию, созданию, дедлайну, SP, приоритету, типу и эпику; индексы проектов по workspace/порядку и владельцу/порядку, ролей по workspace/уровню. Старые документы валидны без заполнения новых полей; индексы строятся Convex при применении схемы. Удаление эпика/колонки разрывает связи сохраняющихся задач, общие функции очищают связанные записи при будущих удалениях. Отдельная очистка старых осиротевших данных не выполнялась.

Браузерные сценарии, переключение языка Clerk, обе темы, узкий экран, ошибки сети и работа двух аккаунтов на тестовом development deployment остаются ручными проверками.
